// CaribbeanTender general browser logic. Authentication is handled in auth.js.
const page = location.pathname.split('/').pop() || 'index.html';
const db = () => window.supabaseClient;

function setYear(){ document.querySelectorAll('.year').forEach(e => e.textContent = new Date().getFullYear()); }
function esc(v=''){ const e=document.createElement('div'); e.textContent=v; return e.innerHTML; }
function money(v){ return new Intl.NumberFormat('en-JM',{style:'currency',currency:'JMD'}).format(Number(v)); }
function fmtDate(v){ if(!v)return ''; return new Intl.DateTimeFormat('en-JM',{dateStyle:'medium'}).format(new Date(`${v}T00:00:00`)); }
function message(text,type='success'){
  let b=document.getElementById('appMessage');
  if(!b){ b=document.createElement('div'); b.id='appMessage'; document.querySelector('main')?.prepend(b); }
  b.className=`notice ${type==='error'?'error':''}`; b.textContent=text;
}
function busy(form,on){ const b=form.querySelector('button[type="submit"]'); if(!b)return; b.disabled=on; b.dataset.label ||= b.textContent; b.textContent=on?'Please wait…':b.dataset.label; }

async function signedUrl(bucket,path){ if(!path)return null; const {data,error}=await db().storage.from(bucket).createSignedUrl(path,300); return error?null:data.signedUrl; }
async function upload(bucket,file){
  if(!file?.size)return null;
  if(file.size>10*1024*1024) throw new Error('Documents must be 10 MB or smaller.');
  const user=await getCurrentUser(); if(!user) throw new Error('Please log in first.');
  const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');
  const path=`${user.id}/${crypto.randomUUID()}-${safe}`;
  const {error}=await db().storage.from(bucket).upload(path,file,{contentType:file.type});
  if(error)throw error; return {path,name:file.name};
}

function wireTenderForm(){
  if(page!=='create-tender.html')return;
  const form=document.querySelector('form'); if(!form)return;
  const [title,description,closingDate,documentInput]=form.querySelectorAll('input,textarea');
  form.addEventListener('submit',async e=>{
    e.preventDefault(); busy(form,true);
    try{
      const user=await getCurrentUser(); if(!user)throw new Error('Please log in first.');
      const doc=await upload('tender-documents',documentInput?.files?.[0]);
      const payload={business_id:user.id,title:title.value.trim(),description:description.value.trim(),closing_date:closingDate.value,status:'open',document_path:doc?.path||null,document_name:doc?.name||null};
      const {data,error}=await db().from('tenders').insert(payload).select('id').single();
      if(error)throw error;
      message('Tender published successfully.');
      setTimeout(()=>location.href=`tender-details.html?id=${data.id}`,400);
    }catch(error){ message(error.message||'Unable to publish tender.','error'); }
    finally{ busy(form,false); }
  });
}


/* Phase 8: Block bid submission when Supplier trial/subscription access has expired. */
async function requireActiveSupplierAccess(){
  if(page!=='submit-bid.html'||!db())return true;

  const user=await getCurrentUser();

  if(!user){
    location.href='login.html';
    return false;
  }

  const {data,error}=await db()
    .from('supplier_profiles')
    .select('subscription_status,trial_ends_at,subscription_ends_at')
    .eq('user_id',user.id)
    .single();

  if(error||!data){
    console.error('Unable to check Supplier access:',error?.message||'Supplier profile not found.');
    message('Unable to verify your Supplier account access. Please try again.','error');
    return false;
  }

  const now=Date.now();
  let allowed=false;

  if(data.subscription_status==='trial' && data.trial_ends_at){
    allowed=new Date(data.trial_ends_at).getTime()>now;
  }

  if(data.subscription_status==='active'){
    allowed=!data.subscription_ends_at ||
      new Date(data.subscription_ends_at).getTime()>now;
  }

  if(!allowed){
    const tenderId=new URLSearchParams(location.search).get('id');
    const returnPart=tenderId?`&tender=${encodeURIComponent(tenderId)}`:'';
    location.href=`subscription.html?reason=trial-expired${returnPart}`;
    return false;
  }

  return true;
}

function wireBidForm(){
  if(page!=='submit-bid.html')return;

  const form=document.querySelector('form');
  if(!form)return;

  const amountInput=document.getElementById('bidAmount');
  const documentInput=document.getElementById('proposalDocument');

  form.addEventListener('submit',async e=>{
    e.preventDefault();

    if(!amountInput || !documentInput){
      message('The bid form is not configured correctly.','error');
      return;
    }

    const amount=Number(amountInput.value);

    if(!Number.isFinite(amount) || amount<=0){
      message('Please enter a valid bid amount using numbers only.','error');
      amountInput.focus();
      return;
    }

    const file=documentInput.files?.[0];

    if(!file){
      message('Please select a proposal document.','error');
      documentInput.focus();
      return;
    }

    const allowedExtensions=['pdf','doc','docx','xls','xlsx'];
    const extension=(file.name.split('.').pop()||'').toLowerCase();

    if(!allowedExtensions.includes(extension)){
      message('Proposal documents must be PDF, Word or Excel files.','error');
      documentInput.value='';
      documentInput.focus();
      return;
    }

    if(file.size>10*1024*1024){
      message('Proposal documents must be 10 MB or smaller.','error');
      documentInput.value='';
      documentInput.focus();
      return;
    }

    busy(form,true);

    try{
      const user=await getCurrentUser();
      if(!user)throw new Error('Please log in first.');

      // Re-check access immediately before accepting/uploading the bid.
      // The database RLS policy remains the authoritative security control.
      const hasSupplierAccess=await requireActiveSupplierAccess();
      if(!hasSupplierAccess)return;

      const tenderId=new URLSearchParams(location.search).get('id');
      if(!tenderId)throw new Error('No tender was selected.');

      const doc=await upload('bid-documents',file);
      if(!doc)throw new Error('The proposal document could not be uploaded.');

      const {error}=await db().from('bids').insert({
        tender_id:tenderId,
        supplier_id:user.id,
        amount:amount,
        document_path:doc.path,
        document_name:doc.name
      });

      if(error){
        await db().storage.from('bid-documents').remove([doc.path]);
        throw error;
      }

      form.reset();
      message('Your bid was submitted successfully.','success');

    }catch(error){
      console.error('Bid submission error:',error);
      message(error.message||'Unable to submit bid.','error');
    }finally{
      busy(form,false);
    }
  });
}

async function profileNames(ids,role){
  const unique=[...new Set(ids.filter(Boolean))];
  if(!unique.length)return {};
  const view=role==='supplier'?'supplier_names_public':'business_names_public';
  const {data,error}=await db().from(view).select('user_id,company_name').in('user_id',unique);
  if(error){console.error('Unable to load company names:',error.message);return {};}
  return Object.fromEntries((data||[]).map(p=>[p.user_id,p.company_name||(role==='supplier'?'Supplier':'Business')]));
}

async function renderTenderRows(){
  const body=document.getElementById('tenderRows'); if(!body||!db())return;
  const today=new Date().toISOString().slice(0,10);
  const {data,error}=await db().from('tenders').select('id,business_id,title,closing_date,status').eq('status','open').gte('closing_date',today).order('closing_date');
  if(error){ body.innerHTML=`<tr><td colspan="5">${esc(error.message)}</td></tr>`; return; }
  const names=await profileNames((data||[]).map(t=>t.business_id),'business');
  body.innerHTML=(data||[]).length?(data||[]).map(t=>`<tr><td>${esc(t.title)}</td><td>${esc(names[t.business_id]||'Business')}</td><td>${fmtDate(t.closing_date)}</td><td><span class="badge">Open</span></td><td><a class="btn" href="tender-details.html?id=${encodeURIComponent(t.id)}">View Details</a></td></tr>`).join(''):'<tr><td colspan="5">No open tenders found.</td></tr>';
}

async function renderDetails(){
  const box=document.getElementById('tenderDetails'); if(!box||!db())return;
  const id=new URLSearchParams(location.search).get('id'); if(!id){box.textContent='No tender was selected.';return;}
  const {data:t,error}=await db().from('tenders').select('*').eq('id',id).single();
  if(error||!t){box.textContent='Tender not found.';return;}
  const names=await profileNames([t.business_id],'business'); const url=await signedUrl('tender-documents',t.document_path);
  const open=t.status==='open'&&t.closing_date>=new Date().toISOString().slice(0,10);
  box.innerHTML=`<h2>${esc(t.title)}</h2><p><strong>Business:</strong> ${esc(names[t.business_id]||'Business')}</p><p><strong>Closing Date:</strong> ${fmtDate(t.closing_date)}</p><p>${esc(t.description)}</p>${url?`<p><strong>Supporting Document:</strong> <a href="${url}" target="_blank" rel="noopener">${esc(t.document_name||'Download')}</a></p>`:''}<div class="actions">${open?`<a class="btn" href="submit-bid.html?id=${encodeURIComponent(t.id)}">Submit Bid</a>`:''}<a class="btn gold" href="tenders.html">Back to Tenders</a></div>`;
}

async function renderMyTenders(){
  const body=document.getElementById('myTenderRows'); if(!body||!db())return;
  const user=await getCurrentUser(); if(!user)return;
  const {data,error}=await db().from('tenders').select('id,title,closing_date,status').eq('business_id',user.id).order('created_at',{ascending:false});
  if(error){body.innerHTML=`<tr><td colspan="4">${esc(error.message)}</td></tr>`;return;}
  let totalBids=0; const rows=[];
  for(const t of data||[]){
    const countRes=await db().from('bids').select('*',{count:'exact',head:true}).eq('tender_id',t.id); const count=countRes.count||0; totalBids+=count;
    rows.push(`<tr><td>${esc(t.title)}</td><td>${fmtDate(t.closing_date)}</td><td>${count}</td><td><a class="btn" href="view-bids.html?id=${encodeURIComponent(t.id)}">View Bids</a></td></tr>`);
  }
  body.innerHTML=rows.join('')||'<tr><td colspan="4">You have not published a tender yet.</td></tr>';
  const s=document.querySelectorAll('.stat strong'); if(s.length>=3){s[0].textContent=(data||[]).length;s[1].textContent=totalBids;s[2].textContent=(data||[]).filter(t=>t.status==='open').length;}
}

async function renderBids(){
  const body=document.getElementById('bidRows');
  if(!body || !db()) return;

  const tenderId=new URLSearchParams(location.search).get('id');
  if(!tenderId) return;

  const {data:bids,error}=await db()
    .from('bids')
    .select('*')
    .eq('tender_id',tenderId)
    .order('created_at');

  if(error){
    body.innerHTML=`<tr><td colspan="4">${esc(error.message)}</td></tr>`;
    return;
  }

  const supplierIds=[...new Set(
    (bids||[]).map(b=>b.supplier_id).filter(Boolean)
  )];

  let supplierNames={};

  if(supplierIds.length){
    const {data:suppliers,error:supplierError}=await db()
      .from('supplier_names_public')
      .select('user_id,company_name')
      .in('user_id',supplierIds);

    if(supplierError){
      console.error('Unable to load supplier company names:',supplierError.message);
    }else{
      supplierNames=Object.fromEntries(
        (suppliers||[]).map(s=>[
          s.user_id,
          s.company_name || 'Supplier'
        ])
      );
    }
  }

  const rows=[];

  for(const bid of bids||[]){
    const documentUrl=await signedUrl('bid-documents',bid.document_path);
    const supplierCompany=supplierNames[bid.supplier_id] || 'Supplier';

    rows.push(`
      <tr>
        <td>${esc(supplierCompany)}</td>
        <td>${money(bid.amount)}</td>
        <td>${new Date(bid.created_at).toLocaleDateString('en-JM')}</td>
        <td>${
          documentUrl
            ? `<a href="${documentUrl}" target="_blank" rel="noopener">${esc(bid.document_name||'Proposal')}</a>`
            : 'Unavailable'
        }</td>
      </tr>
    `);
  }

  body.innerHTML=rows.join('') ||
    '<tr><td colspan="4">No bids have been submitted.</td></tr>';
}


/* Supplier trial / subscription status card */
async function loadSupplierSubscriptionStatus(){
  const card=document.getElementById('subscriptionStatus');
  if(!card||!db())return;

  const user=await getCurrentUser();
  if(!user){
    card.innerHTML='<div class="notice error">Please log in to view your Supplier account status.</div>';
    return;
  }

  const {data,error}=await db()
    .from('supplier_profiles')
    .select('trial_started_at,trial_ends_at,subscription_status,subscription_plan,subscription_started_at,subscription_ends_at')
    .eq('user_id',user.id)
    .single();

  if(error){
    console.error('Unable to load Supplier subscription status:',error.message);
    card.innerHTML='<div class="notice error">Unable to determine your Supplier subscription status.</div>';
    return;
  }

  if(data.subscription_status==='active'){
    const periodText=data.subscription_ends_at
      ? `<p>Current subscription period ends: <strong>${new Date(data.subscription_ends_at).toLocaleDateString('en-JM')}</strong></p>`
      : '';

    card.innerHTML=`
      <div class="subscription-active">
        <strong>✓ SUBSCRIPTION ACTIVE</strong>
        <p>Plan: <strong>${esc(data.subscription_plan||'Supplier Plan')}</strong></p>
        ${periodText}
      </div>`;
    return;
  }

  if(data.subscription_status==='trial' && data.trial_ends_at){
    const end=new Date(data.trial_ends_at);
    const milliseconds=end.getTime()-Date.now();
    const daysRemaining=Math.max(0,Math.ceil(milliseconds/(1000*60*60*24)));

    if(milliseconds>0){
      card.innerHTML=`
        <div class="trial-active">
          <strong>FREE SUPPLIER TRIAL</strong>
          <h2>${daysRemaining} ${daysRemaining===1?'day':'days'} remaining</h2>
          <p>Your free Supplier trial expires <strong>${end.toLocaleDateString('en-JM')}</strong>.</p>
          <a class="btn" href="subscription.html">View Plans</a>
        </div>`;
      return;
    }
  }

  card.innerHTML=`
    <div class="trial-expired">
      <strong>SUPPLIER SUBSCRIPTION REQUIRED</strong>
      <p>Your free Supplier trial has ended. Subscribe to continue submitting bids.</p>
      <a class="btn" href="subscription.html">Subscribe Now</a>
    </div>`;
}

async function renderSupplierStats(){
  if(page!=='supplier-dashboard.html'||!db())return; const user=await getCurrentUser(); if(!user)return;
  const today=new Date().toISOString().slice(0,10);
  const [open,bids,profile]=await Promise.all([db().from('tenders').select('*',{count:'exact',head:true}).eq('status','open').gte('closing_date',today),db().from('bids').select('*',{count:'exact',head:true}).eq('supplier_id',user.id),db().from('supplier_profiles').select('*',{count:'exact',head:true}).eq('user_id',user.id)]);
  const s=document.querySelectorAll('.stat strong'); if(s[0])s[0].textContent=open.count||0;if(s[1])s[1].textContent=bids.count||0;if(s[2])s[2].textContent=profile.count||0;
}

document.addEventListener('DOMContentLoaded',async()=>{
  setYear();
  if(!window.supabaseClient){ message('Supabase did not initialize. Check assets/supabase-config.js and confirm the CDN script loads first.','error'); return; }
  wireTenderForm(); wireBidForm();

  // On submit-bid.html, redirect expired/inactive Suppliers before they use the bid form.
  if(page==='submit-bid.html'){
    const allowed=await requireActiveSupplierAccess();
    if(!allowed)return;
  }

  try{ await Promise.all([renderTenderRows(),renderDetails(),renderMyTenders(),renderBids(),renderSupplierStats(),loadSupplierSubscriptionStatus()]); }
  catch(error){ console.error(error); message(error.message||'The page could not load data from Supabase.','error'); }
});
